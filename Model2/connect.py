...
def main():
	...
	#print(len(nodesSim), len(nodesCFD))
	...
	with open(script_filename,"w") as f:
		...
		for i in range(nodesSim.shape[0]):
			...
			f.write("elm = mdb.models['Model-1'].rootAssembly.instances['Part-1-1'].nodes.getByBoundingBox("+str(x1)+","+str(y1)+","+str(-eps)+","+str(x2)+","+str(y2)+","+str(+eps)+")\n")
		...
		for i in range(nodesSim.shape[0]):
			...
			prep_string += 'Group'+str(i)+', '+str(nodesSim[i].velocity[0])+', '+str(nodesSim[i].velocity[1])+'\\n'
		mdb.models['Model-1'].keywordBlock.insert(str(line_num)+','+prep_string)
		'''
		prep_string = '\\n** \\n** PREDEFINED FIELDS\\n** \\n*INITIAL CONDITIONS, TYPE=MASS FLOW\\n'
		for i in range(nodesSim.shape[0]):
			#atribui a velocidade de cada ponto
			prep_string += 'Group'+str(i)+', '+str(nodesSim[i].velocity[0])+', '+str(nodesSim[i].velocity[1])+'\\n'
		full_str = "modelkwb = mdb.models['Model-1'].keywordBlock\nline_num = 0\nfor n, line in enumerate(modelkwb.sieBlocks):\n	if line == '** PREDEFINED FIELDS':\n		line_num = n\n		break\nprint(line_num)\nprep_string = '*INITIAL CONDITIONS, TYPE=MASS FLOW\\n'\nmdb.models['Model-1'].keywordBlock.insert(line_num,'"+prep_string+"')"
		f.write(full_str)
		'''
		for i in range(nodesSim.shape[0]):
			#atribui a velocidade de cada ponto
			f.write("mdb.models['Model-1'].keywordBlock.insert("+str(30+0*i)+",'\\n** \\n** PREDEFINED FIELDS\\n** \\n*INITIAL CONDITIONS, TYPE=MASS FLOW\\nGroup"+str(i)+", "+str(nodesSim[i].velocity[0])+", "+str(nodesSim[i].velocity[1])+"\\n** Name: Predefined Field-"+str(i)+"   Type: Temperature')\n")
		'''
